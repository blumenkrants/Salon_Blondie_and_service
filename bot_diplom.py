...
def choose_master(bot, update, user_data):
    ...
    elif user_text == 'Маникюрный зал':  

        keyboard_2 = []
        row_2 = []
        rows_2 = []

        info_text_2 = '{} - {} \n {} - {} \n'.format(all_services_2[0], all_price_2[0],
                                                    all_services_2[1], all_price_2[1])
                                                                        

        for i_2 in all_services_2[0:2]:
            row_2.append(InlineKeyboardButton(i_2, callback_data=str(i_2)))
        for i_2 in all_services_2[2:4]:
            rows_2.append(InlineKeyboardButton(i_2, callback_data=str(i_2)))
                
        keyboard_2.append(row_2)
        keyboard_2.append(rows_2)
        reply_markup_2 = InlineKeyboardMarkup(keyboard_2)
        update.message.reply_text(info_text_2, reply_markup=reply_markup_2)
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[RegexHandler('Запись', choose_room, pass_user_data=True),
                      RegexHandler('Парикмахерский зал', choose_master, pass_user_data=True),
                      RegexHandler('Маникюрный зал', choose_master, pass_user_data=True),
